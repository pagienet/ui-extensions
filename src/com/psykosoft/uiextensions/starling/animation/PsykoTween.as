...
package com.psykosoft.uiextensions.starling.animation
{
	...
	/** A PsykoTween animates numeric properties of objects. It uses different transition functions
     ...
     *  of available Transition types, look at the "PsykoTransitions" class.</p> 
     *  
     *  <p>Here is an example of a tween that moves an object to the right, rotates it, and 
     *  fades it out:</p>
     *  
     *  <listing>
     *  var tween:PsykoTween = new PsykoTween(object, 2.0, PsykoTransitions.EASE_IN_OUT);
     ...
     *  
     *  @see Juggler
     *  @see PsykoTransitions
     */ 
    public class PsykoTween extends EventDispatcher implements IAnimatable
    {
        ...
        private var mNextTween:PsykoTween;
         ...
         *  @param time the duration of the PsykoTween
         *  @param transition can be either a String (e.g. one of the constants defined in the
         *         PsykoTransitions class) or a function. Look up the 'PsykoTransitions' class for a   
         *         documentation about the required function signature. */ 
        public function PsykoTween(target:Object, time:Number, transition:Object="linear")
        {
             reset(target, time, transition);
        }

        /** Resets the tween to its default values. Useful for pooling tweens. */
        public function reset(target:Object, time:Number, transition:Object="linear"):PsykoTween
        {
            mTarget = target;
            mCurrentTime = 0;
        ...
        /** The transition method used for the animation. @see PsykoTransitions */
        public function get transition():String { return mTransitionName; }
        public function set transition(value:String):void 
        { 
            mTransitionName = value;
            mTransitionFunc = PsykoTransitions.getTransition(value);
        ...
        public function get nextTween():PsykoTween { return mNextTween; }
        public function set nextTween(value:PsykoTween):void { mNextTween = value; }
        
        // tween pooling
        
        private static var sTweenPool:Vector.<PsykoTween> = new <PsykoTween>[];
        
        /** @private */
        starling_internal static function fromPool(target:Object, time:Number, 
                                                   transition:Object="linear"):PsykoTween
        {
            if (sTweenPool.length) return sTweenPool.pop().reset(target, time, transition);
            else return new PsykoTween(target, time, transition);
        }
        
        /** @private */
        starling_internal static function toPool(tween:PsykoTween):void
        {
